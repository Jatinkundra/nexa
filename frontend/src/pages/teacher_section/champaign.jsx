...
export default function Champaign() {
    ...
    const { createChallenge } = useContext(TransactionContext);

    const handleSubmit = (e) => {
        e.preventDefault();
        console.log(title, problink, creator, reward, deadline, voteperiod);

        // createChallenge(problink, reward, deadline, voteperiod).then((res) => {
        //     console.log("campaign integration completed:" + res);

            axios.post('http://localhost:3001/api/campaign/createCampaign', {
                title: title,
                problink: problink,
                creator: creator,
                reward: reward,
